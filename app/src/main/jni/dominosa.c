...
static game_state *execute_move(const game_state *state, const char *move)
{
    ...
    while (*move) {
        if (move[0] == 'S') {
            ...
            for (i = 0; i < wh2; i++) {
                int id1 = i/2, idn = ((i%2) ? w : 1), id2 = id1 + idn, idi, j;
                if ((idn == 1 ? !(id2%w) : (id2 >= wh)) || ret->grid[id1] != id1 || ret->grid[id2] != id2)
                    continue;
                ...
                if (!used[idi]) {
                    ...
                    for (j = 0; j < wh2; j++) {
                        int jd1 = j/2, jdn = ((j%2) ? w : 1), jd2 = jd1 + jdn;
                        if ((jdn == 1 ? !(jd2%w) : (jd2 >= wh)) || ret->grid[jd1] != jd1 || ret->grid[jd2] != jd2)
                            continue;
